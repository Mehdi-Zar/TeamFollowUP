    ...
    <div className="card stack" style={{ gap: 12 }}>
      <div className="between" style={{ alignItems: "center" }}>
        <div className="inline" style={{ gap: 6 }}>
          <button className="btn-secondary btn-sm" aria-label={t("common.prev")} onClick={() => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() - 1, 1))}>‹</button>
          <button className="btn-secondary btn-sm" onClick={() => setCursor(startOfMonth(new Date()))}>{t("leaves.today")}</button>
          <button className="btn-secondary btn-sm" aria-label={t("common.next")} onClick={() => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() + 1, 1))}>›</button>
        </div>
...
function MineView({ bump, onOpen, onNew }: { bump: number; onOpen: (l: Leave) => void; onNew: () => void }) {
  ...
  // Not loaded is not "no leave": said as such.
  const [loadErr, setLoadErr] = useState<string | null>(null);
  useEffect(() => {
    setLoadErr(null);
    api.get<Leave[]>("/api/leaves?mine=true").then(setMine).catch((e) => setLoadErr(errorText(e)));
    ...
    <div className="stack" style={{ gap: 16 }}>
      {pending.length > 0 && (
        <div className="card stack" style={{ gap: 8 }}>
          ...
          {pending.map((r) => (
            <div key={r.id} className="between item-row" style={{ cursor: "pointer" }} role="button" onClick={() => onOpen(r)} tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onOpen(r); } }}>
  ...
  return (
    <Modal title={leave ? t("leaves.edit") : t("leaves.new")} onClose={onClose} width={480}
      footer={<>
        ...
        <button className="btn" disabled={busy || !typeId || !start || !end || (needsDetail && !detail.trim())} onClick={save}>{busy ? t("common.saving") : t("action.save")}</button>
      </>}>
