...
export default function LoginPage() {
  ...
  // Already signed in: back to the page that asked for it (a path of this app
  // only, never another site), else home.
  const next = params.get("next") || "";
  const back = next.startsWith("/") && !next.startsWith("//") && !next.startsWith("/login") ? next : "/";
  if (!loading && user) return <Navigate to={back} replace />;
  ...
  const ssoButton = (m: Method) => (
    <a key={m.key} className={`btn ${m.primary ? "" : "btn-secondary"} login-method`}
       href={SSO_HREF[m.key] ? `${SSO_HREF[m.key]}${back !== "/" ? `?next=${encodeURIComponent(back)}` : ""}` : "/"}>
