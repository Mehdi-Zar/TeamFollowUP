...
export function SmtpAdmin() {
  ...
  async function test() {
    ...
    try {
      ...
      setTestMsg(r.ok ? t("smtp.test_ok") : t("smtp.test_fail") + (r.error ? ` (${r.error})` : ""));
    } catch (e: any) {
...
export function TrustAdmin() {
  ...
  const ls = useLoadState();
  const load = () => api.get<any>("/api/admin/trust-store").then(setSt).catch(ls.fail);
  useEffect(() => { load(); }, []);
  if (!st) return ls.waiting(load);
  ...
  const caRow = (c: any) => (
    <div key={c.id} className="card stack" style={{ gap: 4, padding: 10 }}>
      ...
      <div className="small muted">{t("trust.issuer")}{t("common.colon")}{c.issuer}</div>
      <div className="small muted">{t("trust.expires")}{t("common.colon")}{formatDate(c.not_after)}</div>
...
export function AuthAdmin() {
  ...
  const { error, wrap } = useErr();
  const roles = ["admin", "tribe_leader", "squad_leader", "contributor", "member"];
    ...
    <div className="stack">
      ...
      <div className="card">
        ...
        <div className="banner stack" style={{ gap: 8, marginTop: 12 }}>
          ...
          <div className="small">
            {t("auth.test")}{t("common.colon")}<a href="/api/auth/saml/metadata" target="_blank">/api/auth/saml/metadata</a>
          </div>
        </div>
        {testPanel("saml")}
        <details style={{ marginTop: 10 }}>
