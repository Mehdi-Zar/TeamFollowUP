...
export function ModerationAdmin() {
  ...
  // A refused load or action is said: an error used to read as "no message".
  const { error, wrap } = useErr();
  function load() {
    api.get<any[]>("/api/feed").then(setPosts).catch((e) => { setPosts([]); wrap(() => Promise.reject(e)); });
  }
  useEffect(() => { load(); }, []);
  const delPost = (id: number) => wrap(async () => { await api.del(`/api/feed/${id}`); load(); });
  const delReply = (id: number) => wrap(async () => { await api.del(`/api/feed/replies/${id}`); load(); });
  const pin = (p: any) => wrap(async () => { await api.put(`/api/feed/${p.id}/pin`, { is_pinned: !p.is_pinned }); load(); });

  if (posts === null) return <div className="spinner">{t("common.loading")}</div>;
  return (
    <div className="stack">
      <div className="banner">{t("mod.title")}</div>
      {error && <ErrorBanner message={error} />}
      {posts.length === 0 && !error && <div className="card muted">{t("mod.none")}</div>}
      {posts.map((p) => (
...
export function OpsAdmin() {
  ...
  const load = () => api.get<any>("/api/admin/runtime").then((r) => { setRt(r); setLoadErr(null); })
    .catch((e) => setLoadErr(errorText(e)));
  useEffect(() => { load(); }, []);

  function fmtUptime(s: number): string {
    const m = Math.floor(s / 60), h = Math.floor(m / 60), d = Math.floor(h / 24);
    if (d > 0) return `${d}${t("ops.day_short")} ${h % 24}h`;
  ...
  if (!rt && loadErr) return (
    <div className="stack">
      <ErrorBanner message={loadErr} />
      <div><button className="btn-secondary btn-sm" onClick={load}>{t("common.retry")}</button></div>
    </div>
  );
  ...
  const rows: Array<[string, any]> = [
    ...
    [t("ops.field.started_at"), formatDateTime(rt.started_at)],
...
export function LogsPanel() {
  ...
  const load = () => api.get<LogsData>(`/api/admin/logs?limit=1000${filter ? `&level=${filter}` : ""}`)
    .then((d) => { setData(d); setPendingLevel((p) => p || d.level); setLastAt(new Date().toLocaleTimeString(lang === "en" ? "en-GB" : "fr-FR")); })
    .catch((e) => { if (!data) wrap(() => Promise.reject(e)); });

  useEffect(() => { load(); /* eslint-disable-next-line */ }, [filter]);
  useEffect(() => {
    if (!autoRefresh) return;
    // Not while the tab is hidden, and never a second read while one is running.
    let inFlight = false;
  ...
  return (
    <div className="card stack" style={{ gap: 12 }}>
      ...
      <div className="row" style={{ gap: 10, alignItems: "flex-end", flexWrap: "wrap" }}>
        ...
        <span className="small muted">{t("ops.log_current")}{t("common.colon")}<span className="strong" style={{ color: LOG_LEVEL_COLOR[data.level] }}>{data.level}</span></span>
      </div>
