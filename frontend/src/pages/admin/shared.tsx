...
/** A panel's first load: `fail` records why it failed, `waiting(retry)` is what
 *  to show until the data is there (a spinner, or the reason with a retry
 *  button, never a spinner that turns forever). */
export function useLoadState() {
  const { t } = useI18n();
  const [loadErr, setLoadErr] = useState<string | null>(null);
  const fail = (e: unknown) => setLoadErr(errorText(e));
  const waiting = (retry: () => void) => loadErr ? (
    <div className="banner banner-red between" role="alert" style={{ alignItems: "center" }}>
      <span>{loadErr}</span>
      <button className="btn-secondary btn-sm" onClick={() => { setLoadErr(null); retry(); }}>{t("common.retry")}</button>
    </div>
