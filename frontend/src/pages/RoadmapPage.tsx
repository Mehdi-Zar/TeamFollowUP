...
import { Dot, Spinner, ErrorBanner, EmptyState } from "../components/ui";
import { Rag, RoadmapStatus } from "../types";

const statusRag = (s: RoadmapStatus): Rag =>
  s === "blocked" ? "red" : s === "at_risk" ? "amber" : "green";
...
/** Milestone line: its status (dot + label for screen readers), the title, and
 *  the EA/GA stage coloured. */
function JalonLine({ it }: { it: RoadmapCellItem }) {
  const { roadmap } = useI18n();
  return (
    <div className="rmv-j" title={[roadmap(it.status), it.dependency].filter(Boolean).join(" | ")}>
      <Dot status={statusRag(it.status)} />{" "}
      {it.title}
      {it.stage && (
...
export default function RoadmapPage() {
  ...
  // The roadmap of a year, chosen here like on every other screen (it had none).
  // The year lives in the URL (?year=): back from a squad page, the same year.
  ...
  useEffect(() => {
    setData(null);
    setError(null);
    // Two years clicked in a row: only the last answer is shown (the heavy
    // matrix of the first could arrive after and sit under the wrong year).
    let alive = true;
    api.get<RoadmapMatrix>(`/api/roadmap/matrix?year=${year}${tribeId ? `&tribe_id=${tribeId}` : ""}`)
      ...
      <div className="inline" style={{ gap: 10, flexWrap: "wrap" }}>
        ...
        <div className="seg">
          {[year - 1, year, year + 1].map((y) => (
            <button key={y} className={y === year ? "active" : ""} aria-pressed={y === year} onClick={() => setYear(y)}>{y}</button>
          ))}
        </div>
        ...
        <ExportMenu docs={["roadmap", "dependencies"]} year={year} />
      </div>
    ),
  }, [isAdmin, tribes, tribeId, year, t]);
                  ...
                  {tb.squads.map((s) => (
                    <tr key={s.squad_id}>
                      <th className="rmv-row">
                        <Link to={`/squads/${s.squad_id}?year=${year}`}>{s.name}</Link>
