...
export default function CommandPalette({ pages }: { pages: { to: string; label: string }[] }) {
  ...
  const items: Item[] = useMemo(() => {
    ...
    const hits = needle ? all.filter((i) => i.label.toLowerCase().includes(needle)) : all;
    moreRef.current = Math.max(0, hits.length - 12);
    ...
    <div className="cmd-overlay no-print" onClick={() => setOpen(false)}>
      <div className="cmd" role="dialog" aria-modal="true" aria-label={t("cmd.placeholder")} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="cmd-list">
          ...
          {/* More than shown: said, so nobody thinks the list is complete. */}
          {moreRef.current > 0 && <div className="cmd-empty">{t("cmd.more", { n: moreRef.current })}</div>}
        </div>
      </div>
    </div>
  );
