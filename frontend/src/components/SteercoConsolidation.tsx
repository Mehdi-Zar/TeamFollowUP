...
export default function SteercoConsolidation({ period, setPeriod, platformId, setPlatformId }: Props) {
  ...
  useEffect(() => {
    ...
    api.get<Entry[]>(`/api/steerco/entries?period=${encodeURIComponent(period)}`)
      ...
      .catch((e) => { setErr(errorText(e)); setEntries([]); });
  ...
  // "waiting on its squad" rather than repeating the name.
  const who = (e: Entry) =>
    e.missing.length === 1 && e.missing[0] === e.platform_name ? t("steerco.its_squad") : e.missing.join(", ");
  const orphans = (entries ?? []).filter((e) => e.unassigned > 0);
    ...
    <>
      ...
      {e.missing.length > 0 && (
        <span className="small" style={{ color: "var(--orange)" }}>
          {t("steerco.waiting_short", { who: who(e) })}
        </span>
      )}
    </>
  );
          ...
          <div className="small" style={{ marginTop: 6 }}>
            {late.map((e) => (
              <div key={e.platform_id} className="muted">
                {t("steerco.waiting_on", { platform: e.platform_name, who: who(e) })}
              </div>
            ))}
          </div>
        )}
        {/* Indicators no squad answers for: nobody will ever fill them, and the
            platform can never be complete. Said here, with the way to fix it. */}
        {orphans.length > 0 && (
          <div className="small" style={{ marginTop: 6, color: "var(--orange)" }}>
            {orphans.map((e) => (
              <div key={e.platform_id}>
                {t("steerco.unassigned_on", { platform: e.platform_name, n: e.unassigned })}{" "}
