...
export function BackfillGrid({ platformId, period, template, editable }:
  ...
  // The history did not load: the grid would look empty and saving it would
  // blank the months already entered, so saving waits for a successful load.
  ...
  }, [open, platformId, period, loadTick]);
  ...
  async function save() {
    if (loadErr) return;
    setBusy(true); setMsg(null); setMsgBad(false);
    try {
      ...
      setMsg(errorText(e)); setMsgBad(true);
          ...
          <div className="inline" style={{ justifyContent: "flex-end", gap: 10 }}>
            {loadErr && (
              <span className="small" role="alert" style={{ color: "var(--red)" }}>
                {loadErr} <button type="button" className="btn-ghost btn-sm" onClick={() => setLoadTick((n) => n + 1)}>{t("common.retry")}</button>
              </span>
            )}
            {msg && <span className="small" role="status" style={{ color: msgBad ? "var(--red)" : "var(--green)" }}>{msg}</span>}
            <button type="button" className="btn-sm" disabled={busy || !!loadErr} onClick={save}>{t("steerco.f.backfill_save")}</button>
          </div>
        </>
      )}
    </div>
