...
// A "<key>_one" plural form is reached through its base key (see t() in i18n.tsx).
const reachable = (k: string): boolean =>
  named.has(k) || prefixes.some((p) => k.startsWith(p)) || (k.endsWith("_one") && reachable(k.slice(0, -4)));
