...
export interface Persona {
  ...
  /** Administration tabs handed to this persona (tribe leader only). */
  admin_tabs?: string[];
}
...
export interface Member {
  ...
  /** Key to the login account: the link (user_id) is made from it. */
  email?: string | null;
  user_id?: number | null;
...
export interface SquadDetail extends Squad {
  /** Combien d'engagements la squad a pris pour l'annee (portee « squad »). */
  otd_count?: number;
  leader?: LeaderInfo | null;
  /** Identities of the co-leaders, for display next to the leader. */
  co_leaders?: LeaderInfo[];
  /** Identities of the contributors (they do the squad's reporting). */
  contributors?: LeaderInfo[];
  year: number;
...
export interface ModulesConfig {
  ...
  squad_content: { enabled: boolean; roadmap: boolean; kpis: boolean };
...
export interface PublicConfig {
  ...
  app_subtitle: string;
  default_lang: "fr" | "en";
  /** May everyone pick their own language? Off: the instance reads in default_lang. */
  lang_switch?: boolean;
  default_year: number;
