...
# ---- management OTD: squad leader forbidden, tribe allowed ----
# (The squad objectives were retired: the OTD is the only commitment.)
s, _ = req(sara, "POST", "/api/otds", {"tribe_id": tA, "year": 2026, "title": "x"})
check("squad leader cannot set a management OTD", s == 403, s)
s, o = req(tribe, "POST", "/api/otds", {"tribe_id": tA, "year": 2026, "title": "OTD test"})
check("tribe leader sets a management OTD", s == 201, (s, o))
if s == 201:
    req(tribe, "DELETE", f"/api/otds/{o['id']}")
...
s, _ = req(hugo, "POST", "/api/roadmap-items", {"squad_id": gid, "year": 2026, "quarter": 1, "title": "x", "theme": "Landing Zones"})
