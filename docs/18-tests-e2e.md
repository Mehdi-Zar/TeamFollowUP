...
| `tests/admin-https.spec.ts` | sur un déploiement où l'infrastructure termine le TLS, le mode de service par défaut : le panneau du certificat servi est masqué et dit pourquoi, et les **autorités approuvées** sont là quand même, avec des boutons actifs. C'est la régression qui rendait impossible l'import d'une AC interne sans basculer le mode de service, donc sans changer de port |
