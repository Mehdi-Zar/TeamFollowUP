...
> **Mise a jour ([31](31-coherence-saisie-et-reporting.md)).** Les objectifs de squad sont
> retires : l'OTD est le seul engagement. Celui du management se regle dans Mes squads, a
> l'etape OTD. Quiconque dirige une squad (meme un tribe leader) en fait le reporting, avec
> ses co-leaders et ses contributeurs. Les passages ci-dessous qui parlent d'objectifs
> annuels, de l'etape Options ou du chiffre « objectifs rouges » decrivent l'etat d'avant.
...
### Et l'etape des engagements est revenue, parce qu'il y a desormais a y faire

La regle n'a pas change, c'est l'objet qui a change. Quand l'etape est partie,
un engagement se fixait uniquement par le tribe leader : son lecteur n'y pouvait
rien, donc elle n'avait pas lieu d'etre. Depuis, l'engagement a **deux portees**
(voir [ADR-0015](adr/0015-otd-has-two-scopes.md)), et celle de la squad est prise
...
Un troisieme bloc, **les autres engagements de la tribe** replies et en lecture, a
ete retire : il montrait dans la saisie d'une squad des engagements qui ne la
concernent pas, ce qui brouillait l'etape. Il repondait pourtant a un piege, qui
reste vrai et que l'API couvre toujours. Un engagement management n'etait
visible d'un squad leader que s'il lui etait assigne nommement, ou si un de ses
jalons y etait deja rattache. Or **seul le tribe leader rattache un jalon** :
l'engagement pose sur une squad restait donc invisible a celui qui devait le
tenir tant qu'un autre n'avait pas agi, et rien ne le lui signalait. Un
engagement qu'on ignore ne peut pas etre tenu, ni conteste. Un squad leader lit
maintenant **tous les engagements management de sa tribe**, sans en ecrire aucun ;
la frontiere reste la tribe.

La liste de controle de l'envoi gagne une ligne, « au moins un engagement pris
par la squad », **non bloquante** comme les autres : une squad a le droit de
n'avoir pris aucun engagement propre. Elle compte la seule portee squad, parce
qu'un engagement pose par le management n'est pas un engagement qu'on a pris
(`SquadDetail.otd_count`).

**Un co-leader dirige aussi.** Le serveur le sait depuis toujours
(`deps.leads_this_squad` compte le leader nomme et ses co-leaders), l'ecran ne
lisait que `leader_user_id` : un co-leader avait tous les droits d'ecriture a
l'API et aucun bouton pour les exercer, sur les jalons, les messages cles et
l'engagement de sa squad. `perms.leadsSquad` et `perms.canEditSquad` passent
desormais par la meme regle que le serveur.
...
| Roadmap | tribe, nom, avancement | la matrice, qui est deja une vue |
