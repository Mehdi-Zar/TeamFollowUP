...
| **squads** | id, tribe_id→tribes, name, description, leader_user_id→users, display_order, **kpis_enabled**, **steerco_enabled**, **budget_enabled**, **squad_type** (product/transverse), **products (JSON)**, **hardware (JSON)** | `squad_type` est **inerte**: il annonce un style de reporting (roadmap pour `product`, initiatives et OTD pour `transverse`) qu'aucun ecran ne lit. Il a quitte le formulaire de « Mes squads » pour ne plus passer pour un reglage actif, la colonne restant en base et l'import continuant de la remplir. `steerco_enabled` est **derive** de l'appartenance a une plateforme dont le steerco est actif (voir [15](15-steerco.md)), jamais pose a la main |
| **squad_coleaders** | squad_id→squads, user_id→users (both ON DELETE CASCADE), pk(squad, user) | co-leaders of a squad: the same rights as `leader_user_id` over that squad, without being its named leader. One helper (`deps.leads_this_squad`) is the single definition, so a right cannot be opened in one screen and forgotten in another |
...
| **otds** | id, tribe_id→tribes, year, title, description, **budget_ref**, **committed_date**, owner_user_id→users, display_order, **scope** (management\|squad), **squad_id→squads** | engagement date. `scope` dit qui le possede: `management` est fixe par le tribe leader (comportement historique, valeur des lignes existantes), `squad` est pris par le squad leader sur une squad qu'il dirige. La portee est fixee a la creation et ne se modifie pas: un squad leader qui pourrait basculer son engagement en `management` s'attribuerait un droit d'ecriture sur un objet du tribe leader. Chaque portee a son propre lien vers les jalons (`otd_id` contre `squad_otd_id`); le statut reste derive |
...
| **roadmap_items** | id, squad_id→squads, year, quarter (1-4), title, **theme** (lane), **release_stage** (EA\|GA), description, success_criteria, user_benefit, dependencies (text), **dependency_kind** (text/squad/tribe), dependency_squad_id→squads, dependency_tribe_id→tribes, risks, owner, status (on_track/at_risk/blocked/done), display_order, **objective_id→objectives**, **initiative_id→initiatives**, **otd_id→otds**, **squad_otd_id→otds** | le "jalon". `initiative_id` dit quelle initiative il sert, et c'est ce que la frise des exports suit. Le chemin passait par `objective_id`, dont aucun ecran ne posait le maillon suivant: toute frise montrait des lignes d'initiative vides. `objective_id` reste, il dit a quel objectif le jalon repond, ce qui n'est pas la meme question. Les deux liens d'engagement sont distincts parce qu'un jalon sert en general les deux a la fois: avec un lien unique, le dernier qui rattache defaisait le travail de l'autre sans le lui dire |
| **quarter_progress** | id, squad_id→squads, year, quarter, progress_pct, comment, **uniq(squad,year,quarter)** | annual % = mean of the quarters that have milestones (`status.annual_mean`) |
| **kpis** | id, squad_id→squads, name, unit, target_value, current_value, trend_status (on_target/under_pressure/missed), comment | |
| **report_snapshots** | id, squad_id→squads, submitted_by_user_id→users, submitted_at, **payload (JSON)**, cycle_label | immutable submission snapshots. Le payload porte de quoi comparer deux soumissions **et** rejouer un document date (objectifs, jalons avec leurs rattachements d'engagement, avancement, KPI, engagements OTD, initiatives, messages cles, moral, statut). Pas le budget: la saisie se lit par tout utilisateur qui voit la squad, ses chiffres non. Voir [29](29-versions-des-documents.md) |
...
| **platforms** | id, tribe_id→tribes (ON DELETE CASCADE), name, description, display_order, steerco_enabled, **template (JSON)**, **uniq(tribe,name)** | what the steering committee looks at: one platform, one slide. `template` lists the slide's KPI cards and SLA columns and, for each, the contributing squad that owes the figure. See [15](15-steerco.md) |
| **platform_contributors** | platform_id→platforms, squad_id→squads (both ON DELETE CASCADE), pk(platform, squad) | which squads feed a platform. Many-to-many: a squad may serve several platforms |
| **steerco_entries** | id, platform_id→platforms (ON DELETE CASCADE), **period** ("YYYY-MM"), **data (JSON)**, updated_at, updated_by_user_id→users, **uniq(platform,period)** | one monthly steering-committee snapshot (KPI counts, the month's SLA per COTS, incident count, events), positional against the platform's template. Raw values only: variations, SLA colours and the calendar-year charts (January to December) are derived at render time. See [15](15-steerco.md) |
...
| `trust` | `trustconfig.py` | the trusted-authority store, roots and intermediates, merged with the public roots into the outbound trust bundle under `CERT_DIR` |
