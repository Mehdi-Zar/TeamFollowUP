...
> the app actually runs (sovereign cloud / GKE, possibly air-gapped) has **no dev tooling and
...
| | **Dev world** (your laptop) | **Run world** (sovereign cloud / prod) |
|---|---|---|
...
### 4.b - Internal registry (sovereign-cloud Artifact Registry reachable from prod)

If the run world can pull from an **internal** registry (no public internet, but
an Artifact Registry inside the perimeter - the sovereign-cloud case), push there instead of
...
See [Deployment Guide §6 (sovereign cloud, air-gapped)](12-deployment-guide.md) for the full
...
> ⚠️ **The port the image binds.** The container serves a **single** port, plain
> **HTTP** on `HTTP_PORT` (8000 by default); the Gateway/ALB terminates TLS and does
> the HTTP→HTTPS redirect (deployment guide §6.9, Step 3). Port mappings, monitoring probes
> and the K8s `containerPort` / probe `scheme` all point at plain HTTP :8000.
>
...
- **Internal Artifact Registry (recommended for a sovereign cloud).** Push `:1.1.0` to the
  ...
  approach the sovereign-cloud section is built around.
