...
already runs on **GKE** (including sovereign-cloud GKE), where a **keyless** identity is
...
- **`google-auth` owns token acquisition.** Re-implementing the STS and IAM
  ...
  handles ADC, WIF, impersonation and a sovereign GCP universe. It is the one new
  dependency.
...
- **Universe-aware.** `universe_domain` (explicit config > credentials > default)
  ...
  endpoint too (`iamcredentials.sovereign-cloud.example` for sovereign-cloud impersonation).
