...
class TribeOut(ORMModel):
    ...
    description: Optional[str] = None
    display_order: int
    # How many squads it counts (the org chart's tribe chips show it to everyone).
    squads_count: int = 0
    # Its tribe leaders' names (Administration > Tribes shows who leads it).
    leaders: list[str] = []
...
class MemberCreate(BaseModel):
    """Fields to add a person to a squad's org chart.

    The email is the key to the login account (see memberlink): with it, the name
    is optional (first/last name, else the account's name, else read from the
    email). Without an email, a name is required."""
    squad_id: int
...
class MemberUpdate(BaseModel):
    """Partial edit of a squad member. A new email re-links the account."""
...
class SquadUpdate(BaseModel):
    ...
    # Contributors do the squad's reporting. Named by the tribe leader or by the
    # squad's own leadership.
    contributor_user_ids: Optional[list[int]] = None
...
class SquadOut(ORMModel):
    ...
    # Who fills in the reporting beside the leadership.
...
class SquadDetail(SquadOut):
    ...
    contributors: list[LeaderInfo] = []
    year: int
    annual_progress: int
    freshness: dict
    counts: dict
    # Combien d'engagements la squad a pris pour l'annee. La page de saisie le dit
    # dans sa liste de controle, au meme titre que les jalons ou les KPI: ce qui
    # n'est pas rappele avant d'envoyer est ce qu'on oublie de remplir.
    otd_count: int = 0
...
class SnapshotMeta(ORMModel):
    ...
    squad_id: int
    submitted_by_user_id: Optional[int] = None
    submitted_by_name: Optional[str] = None  # who submitted, as the history shows it
    submitted_at: datetime
    cycle_label: str
...
class FeedPostOut(BaseModel):
    ...
    # What the caller may do on this post, decided by the server (feed._moderates)
    # so the screen never offers a button that ends in a 403.
