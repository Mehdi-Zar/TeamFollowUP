...
def ensure_breakglass(db: Session, commit: bool = True) -> None:
    ...
    ``commit=False`` only flushes, for a caller inside a larger transaction (a data
    reset or restore): a commit here made that operation half done if what
    followed failed.
    """
