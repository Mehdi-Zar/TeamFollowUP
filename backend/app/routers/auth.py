...
def my_permissions(request: Request, db: Session = Depends(get_db), user: User = Depends(get_current_user_any_status)):
    ...
    from ..personasconfig import user_caps, get_personas
    payload = permissions_payload(user, db)
    # Persona capabilities drive section access in the SPA (and a few endpoints).
    payload["capabilities"] = user_caps(db, user)
    # The names of the custom personas, so a role key never shows raw on screen.
    payload["persona_labels"] = {p["key"]: p["label"] for p in get_personas(db) if not p.get("builtin")}
...
def stop_impersonation(response: Response, request: Request,
                       db: Session = Depends(get_db), user: User = Depends(get_current_user_any_status)):
...
def _safe_next(value) -> str:
    """A path inside the app to return to after signing in, or "/". Never an
    absolute URL, a protocol-relative one, the API or the login page itself."""
    v = str(value or "")
    if (not v.startswith("/") or v.startswith("//") or v.startswith("/\\")
            or v.startswith("/api") or v.startswith("/login")):
        return "/"
    return v
...
async def oidc_login(request: Request, db: Session = Depends(get_db)):
    ...
    request.session["_next"] = _safe_next(request.query_params.get("next"))
