...
def _scope_tribe(user: User) -> int | None:
    """None = all tribes (admin); otherwise the caller's own tribe (none if they have none)."""
...
def create_type(payload: LeaveTypeIn, db: Session = Depends(get_db), user: User = Depends(require_admin)):
    ...
    lt = LeaveType(label=payload.label.strip(), color=payload.color, is_active=payload.is_active,
                   display_order=payload.display_order, requires_detail=payload.requires_detail)
    db.add(lt)
    db.flush()
...
def set_config(payload: LeaveConfigIn, tribe_id: int | None = Query(default=None),
    ...
    """
    user = acting_manager(db, user, "leaves")
    tribe = _resolve_tribe(db, user, tribe_id)
...
def update_leave(leave_id: int, payload: LeaveUpdate, db: Session = Depends(get_db),
    ...
    data = update_data(payload, Leave)
...
def decide_leave(leave_id: int, payload: LeaveDecisionIn, db: Session = Depends(get_db),
    ...
    # Same rule as the can_decide flag the screen reads: a cancelled or already
    # decided leave is not decided again through here.
    if lv.status != "pending":
        raise HTTPException(status_code=409, detail="Cette absence a déjà été traitée")
...
def overlaps(
    ...
    for sq in squads:
        ...
        day = date_from
        while day <= date_to:
            # People, not leaves: two leaves of one person on the same day count once.
            present = sorted({r.user_id for r in rows if r.start_date <= day <= r.end_date})
            if len(present) >= threshold:
                out.append(LeaveOverlapDay(
                    squad_id=sq.id, squad_name=sq.name, day=day, count=len(present),
                    names=[_name(db, uid, cache) or "" for uid in present]))
