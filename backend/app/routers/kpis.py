...
from ..deps import assert_can_report, record_audit, require_module, require_writer, update_data
...
def create_kpi(payload: KpiCreate, db: Session = Depends(get_db),
    ...
    POST /api/kpis
    Access: writer role + reporting rights on the target squad (assert_can_report).
    ...
    assert_can_report(db, user, payload.squad_id)
    ...
    db.commit()
    db.refresh(item)
    notify_change(item.squad_id, "kpi", user)
    return item
...
def update_kpi(kpi_id: int, payload: KpiUpdate, db: Session = Depends(get_db),
    ...
    assert_can_report(db, user, item.squad_id)
    data = update_data(payload, Kpi)
