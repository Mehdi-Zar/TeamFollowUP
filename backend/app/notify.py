...
def _emit(db: Session, smtp: dict, user: User, kind: str, actor: str, excerpt: str, link: str, subject: str) -> None:
    db.add(Notification(user_id=user.id, kind=kind, actor_name=actor, excerpt=excerpt, link=link))
    if user.email_notifications and user.email and smtp.get("enabled"):
...
def notify_new_post(db: Session, post: FeedPost) -> None:
    ...
    for user in db.scalars(q).all():
        _emit(db, smtp, user, "tweet", actor, excerpt, "/fil", f"Nouveau message — {actor}")
...
def notify_reply(db: Session, post: FeedPost, reply: FeedReply) -> None:
    ...
    _emit(db, smtp, target, "reply", actor, _excerpt(reply.content), "/fil", f"Réponse à votre message — {actor}")
