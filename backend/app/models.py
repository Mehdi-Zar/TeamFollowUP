...
class User(Base):
    ...
    # Squads whose reporting this user fills in, without leading them.
    contributed_squads: Mapped[list["Squad"]] = relationship(secondary="squad_contributors",
...
# Contributors of a squad: they fill in and submit its weekly reporting, and do
# nothing else (the set-up, team, budget, committees, stays with its leadership).
squad_contributors = Table(
    "squad_contributors",
    Base.metadata,
    Column("squad_id", ForeignKey("squads.id", ondelete="CASCADE"), primary_key=True),
    Column("user_id", ForeignKey("users.id", ondelete="CASCADE"), primary_key=True),
...
class Squad(Base):
    ...
    # Who does the reporting beside the leadership (see deps.reports_for_squad).
    contributors: Mapped[list["User"]] = relationship(secondary=squad_contributors,
                                                      back_populates="contributed_squads")

    @property
    def contributor_user_ids(self) -> list[int]:
        """The contributors as plain ids, read straight by the API schemas."""
        return [u.id for u in self.contributors]
    ...
    # Les engagements que la squad a pris elle-meme (scope « squad »). Ceux du
    # management ne sont pas ici: ils portent sur une personne et sur des jalons,
    # pas sur la squad, et c'est toute la difference entre les deux portees.
    #
    # `passive_deletes` laisse la base faire ce que sa cle etrangere promet deja
    # (ON DELETE CASCADE). Sans lui, SQLAlchemy, qui connait desormais ce lien,
    # viderait `squad_id` avant la suppression: l'engagement survivrait a la squad
    # qui l'avait pris, sans portee ni proprietaire, visible de personne.
    otds: Mapped[list["Otd"]] = relationship(back_populates="squad", passive_deletes=True,
                                             foreign_keys="Otd.squad_id")
...
class Member(Base):
    ...
    # The key to the login account (lowercase): the link below is made from it, at
    # once or when the account appears (see memberlink).
...
class OrgNode(Base):
    ...
    display_order: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    # The linked squad, read for its current name (loaded with the node).
    squad: Mapped["Squad | None"] = relationship(foreign_keys=[squad_id], lazy="selectin")
...
class RoadmapItem(Base):
    ...
    dependency_squad_id: Mapped[int | None] = mapped_column(ForeignKey("squads.id", ondelete="SET NULL", name="fk_roadmap_dep_squad"), nullable=True)
    dependency_tribe_id: Mapped[int | None] = mapped_column(ForeignKey("tribes.id", ondelete="SET NULL", name="fk_roadmap_dep_tribe"), nullable=True)
    ...
    def dependency_to_text(self, label: str) -> None:
        ...
        link just cannot outlive the row it points to."""
        self.dependency_kind = "text"
