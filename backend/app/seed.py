...
9 squads across 2 tribes, OTD commitments, a quarterly
...
def run_seed(db: Session) -> None:
    ...
    feature (roadmap, KPIs, org chart, feed, snapshots, budgets, OTD).
    ...
    def members(s, people):
        ...
        for i, (name, role_title, user_id) in enumerate(people):
            email = db.get(User, user_id).email.lower() if user_id else None  # the link's key
            db.add(Member(squad_id=s.id, full_name=name, role_title=role_title, email=email,
                          user_id=user_id, display_order=i))
