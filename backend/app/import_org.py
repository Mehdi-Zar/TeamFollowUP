...
flow works locally and in production. ``download_org_template`` / the CLI
...
def _get_or_create_user(db: Session, email: str | None, name: str | None,
    ...
    if name:
        u.display_name = name
    # The higher role wins: the file never demotes (a tribe leader also listed as
    # a squad's leader stays tribe leader).
    rank = {"member": 0, "contributor": 1, "squad_leader": 2, "tribe_leader": 3, ADMIN: 4}
    if rank.get(role, 0) > rank.get(u.role, 0):
        u.role = role
    u.tribe_id = tribe_id
    return u, False
...
def import_org(db: Session, data: dict) -> dict:
    ...
    year = int(data.get("year") or reference_year(db))
    ...
    def _referenced_squad(name: str | None) -> Squad | None:
        ...
        if sq is None:
            msg = (f"Squad « {name} » introuvable dans la tribe {tribe.name} : "
...
def _read_xlsx(path: str) -> dict:
    """Parse the org workbook (sheets Tribe / Squads / Initiatives / OTD /
    ...
    # The sheet is "Tribe"; files made from the older template call it "Tribu".
    tribe_rows = rows("Tribe", key_col=1) or rows("Tribu", key_col=1)
...
def build_template_workbook():
    ...
    build(wb.active, "Tribe",
          ["Annee", "Nom de la tribe", "Description", "Tribe leader (nom)", "Tribe leader (email)"],
          [[2026, "Cloud Platform", "Socle cloud souverain", "Nadia Khaldi", "nadia@exemple.com"]])
    build(wb.create_sheet(), "Squads",
          ...
          [["Portal", "product", "Leo Martin", "leo@exemple.com", "Portal self-service", "", "oui", "non"],
           ["GCP / Sovereign", "product", "Sara Dubois", "sara@exemple.com", "", "", "oui", "non"],
           ["Run & Operation", "transverse", "Paul Lemoine", "paul@exemple.com", "", "", "non", "non"]])
    build(wb.create_sheet(), "Initiatives",
          ["Titre", "Squad concernee", "Porteur", "Echeance (AAAA-MM-JJ)", "Description"],
          [["Souverainete cloud & FinOps", "Portal", "Nadia Khaldi", "2026-12-31", "Reduire le cout du socle cloud."]])
    build(wb.create_sheet(), "OTD",
          ["Titre", "Squad concernee (engagement pris sur son leader)", "Date d'engagement (AAAA-MM-JJ)", "Description"],
          [["Data lake GCP en production", "GCP / Sovereign", "2026-09-30", "Mise en production du data lake."]])
    build(wb.create_sheet(), "Plateformes",
          ["Nom", "Squads contributrices (separees par virgule)", "Description"],
          [["Socle GCP", "GCP / Sovereign, Run & Operation", "Une plateforme alimentee par plusieurs squads."],
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not data or "tribe" not in data or not (data.get("tribe") or {}).get("name"):
        log.error("Le fichier doit définir au moins une tribe (onglet 'Tribe' rempli).")
        sys.exit(1)
    db = SessionLocal()
    try:
        import_org(db, data)
