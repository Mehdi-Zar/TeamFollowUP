...
Keeps the example squad names but generates fake users, members, OTD,
...
that sometimes carry three, milestones wired to initiatives so the timeline has
...
SQUADS = [
    ...
    ("GCP / Sovereign", "Yanis Petit", "yanis.gcp", "red", "Cloud Providers"),
...
def run(db: Session) -> None:
    ...
    squads = []
    squad_refs = []  # (squad, squad_type, unused, jalons): for initiatives/OTD wiring
    for order, (name, leader_name, slug, profile, domain) in enumerate(SQUADS, start=1):
        ...
        squad_refs.append((s, squad_type, [], squad_jalons))
        ...
        for mi, (fn, rt, uid) in enumerate(people):
            db.add(Member(squad_id=s.id, full_name=fn, role_title=rt, user_id=uid,
                          email=leader.email.lower() if uid else None, display_order=mi))
    ...
    for idx, (s, profile, domain, leader) in enumerate(squads):
        ...
        for label, delta in (("Soumission précédente", days_ago + 21), ("Dernière soumission", days_ago)):
            db.add(ReportSnapshot(squad_id=s.id, submitted_by_user_id=s.leader_user_id,
                                  ...
                                  payload=build_payload(db, s, year), cycle_label=label))

    # ---- Feed (tweet zone) ----
    by_name = {s.name: (s, leader) for s, _, _, leader in squads}
    gcp_s, gcp_l = by_name["GCP / Sovereign"]
    ...
    posts = [
        FeedPost(tribe_id=tribe.id, author_user_id=gcp_l.id, kind="incident", squad_id=gcp_s.id, is_pinned=True,
                 content="Incident sur l'ingestion GCP / Sovereign : pipeline en pause, investigation en cours.",
    ...
    if plain_members:
        ...
        db.add(Notification(user_id=m.id, kind="tweet", actor_name=gcp_l.display_name,
                            excerpt="Incident sur l'ingestion GCP / Sovereign…", link="/fil",
