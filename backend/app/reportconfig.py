...
Stored as one JSON blob in app_settings['weekly_report'] for the admin's (all
tribes), and one per tribe in app_settings['weekly_report:tribe:<id>'] for the
schedule a tribe leader runs for their own tribe. Both have the same shape and are
sent by the same code (report.send_due_weekly_reports).
"""
import json
...
def key_for(tribe_id: int | None) -> str:
    """The app_settings key of the admin schedule (None) or of a tribe's."""
...
def _defaults() -> dict:
    ...
    return {
        ...
        # What the fixed recipients get: the whole document in one mail
        # (global_doc), and/or one mail per squad with that squad's document only
        # (per_squad). Both can be on.
        "global_doc": True,
        "per_squad": False,
        # Each squad's leader and co-leaders receive their own squad's document.
        "squad_leaders": False,
        # The squads the per-squad mails cover (empty = every squad in scope).
        "squad_ids": [],
...
def set_report(db: Session, patch: dict, tribe_id: int | None = None) -> dict:
    ...
    cfg["global_doc"] = bool(cfg.get("global_doc", True))
    cfg["per_squad"] = bool(cfg.get("per_squad", False))
    cfg["squad_leaders"] = bool(cfg.get("squad_leaders", False))
    ids = []
    for x in cfg.get("squad_ids") or []:
        try:
            ids.append(int(x))
        except (TypeError, ValueError):
            continue
    cfg["squad_ids"] = sorted(set(ids))
    if tribe_id is not None:
        # The digest sends every tribe's report to its tribe leader: an admin
        # decision, meaningless inside one tribe's own schedule.
        cfg["tribe_leader_digest"] = False
