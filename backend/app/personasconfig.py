...
# Every Administration tab, in the order of the admin menu. The admin chooses, per
# persona, which of them it gets (Admin > Personas); the server follows (tabaccess).
...
def _default_caps(key: str) -> dict:
    ...
    if key == "tribe_leader":
        ...
        # The squad's reporting, for the squads they are named contributor of.
        caps.update(dashboard=True, roadmap=True, org=True, feed=True, reporting=True, leaves=True)
    elif key == "member":
...
def _builtin_personas() -> list[dict]:
    ...
    return [{"key": k, "label": k, "builtin": True, "caps": _default_caps(k),
             "admin_tabs": list(_DEFAULT_TABS.get(k, []))} for k in BUILTINS]
    ...
    if row:
        ...
        for sp in stored if isinstance(stored, list) else []:
            ...
            if key in by_key:
                ...
                if key != "admin" and "admin_tabs" in sp:
                    by_key[key]["admin_tabs"] = _clean_tabs(sp["admin_tabs"])
                ...
                personas.append({"key": key, "label": sp.get("label") or key,
                                 ...
                                 "admin_tabs": _clean_tabs(sp.get("admin_tabs"))})
    return personas
...
def set_personas(db: Session, payload: list[dict]) -> list[dict]:
    ...
    for k in BUILTINS:
        src = incoming.get(k, {})
        if k == "admin":
            tabs = []  # the admin holds every tab, always (admin_tabs_for)
        elif "admin_tabs" in src:
            tabs = _clean_tabs(src.get("admin_tabs"))
        else:
            tabs = list(_DEFAULT_TABS.get(k, []))
    ...
    for p in payload if isinstance(payload, list) else []:
        ...
        out.append({"key": key, "label": (p.get("label") or key), "builtin": False,
                    "caps": _normalize_caps(p.get("caps")), "admin_tabs": _clean_tabs(p.get("admin_tabs"))})
...
def can(db: Session, user, capability: str) -> bool:
    ...
    """
    return bool(user_caps(db, user).get(capability, False))


def user_caps(db: Session, user) -> dict:
    """The persona's capabilities, plus "reporting" for anyone who leads a squad
    or is named its contributor.

    Leading a squad is a job, not a persona: a tribe leader or an admin named at
    the head of a squad must be able to fill in and submit its reporting, whatever
    the persona table says about their role. Same for a contributor.
    """
    caps = dict(persona_caps(db, user.role))
    if not caps.get("reporting") and getattr(user, "id", None) is not None:
        from .deps import contributed_squad_ids, led_squad_ids
        if (db.scalars(led_squad_ids(db, user).limit(1)).first() is not None
            ...
            caps["reporting"] = True
    # Whoever leads a squad manages it from "Mes squads", whatever their persona.
    if not caps.get("mysquads") and getattr(user, "id", None) is not None:
...
def admin_tabs_for(db: Session, user) -> list[str]:
    """The Administration tabs this user may open: every one for an admin (except
    "tribe", their menu lists all tribes), the ticked ones for any other persona."""
    from .rbac import ADMIN_TABS
    if user.role == "admin":
