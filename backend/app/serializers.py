...
def annual_progress(squad: Squad, year: int) -> int:
    """Annual progress (0..100), see status.annual_progress_pct."""
...
def squad_detail(squad: Squad, year: int, threshold: int, privileged: bool = False) -> SquadDetail:
    ...
    return SquadDetail(
        ...
        mood=squad.mood,
        mood_at=squad.mood_at,
        mood_comment=squad.mood_comment,
        co_leader_user_ids=[u.id for u in squad.co_leaders],
        contributor_user_ids=[u.id for u in squad.contributors],
        co_leaders=co_leader_infos(squad),
        contributors=[LeaderInfo(id=u.id, display_name=u.display_name, email=u.email)
                      for u in sorted(squad.contributors, key=lambda x: x.display_name or "")],
        year=year,
...
def build_org_tree(nodes: list[OrgNode], squad_status: dict[int, str]) -> list[OrgNodeTree]:
    ...
    def build(node: OrgNode) -> OrgNodeTree:
        ...
        return OrgNodeTree(
            ...
            # A box linked to a squad reads the squad's current name (a rename
            # used to leave the old one in the chart).
            title=(node.squad.name if node.squad_id and getattr(node, "squad", None) else node.title),
