...
def _universe_from(cfg: dict, creds) -> str:
    ...
    public Google Cloud default (`googleapis.com`). For a sovereign cloud (partner GCP universe)
    this is `sovereign-cloud.example`, so endpoints become e.g. `storage.sovereign-cloud.example`.
    """
    return (
...
def _service_endpoint(service: str, universe: str) -> str:
    """e.g. ("storage", "sovereign-cloud.example") -> "https://storage.sovereign-cloud.example"."""
...
def _credentials(cfg: dict, scopes: list[str]):
    ...
    # Honour the universe for the IAM Credentials endpoint (a sovereign cloud uses its own).
...
def _upload_gcs(cfg: dict, entries: list[dict]) -> tuple[bool, str]:
    ...
    Uses the media-upload REST endpoint on the resolved universe host (so a sovereign cloud
    sovereign cloud works), authenticated with a keyless-first bearer token.
    """
