"""One-shot: wipe demo/org content and load an example Cloud Platform org
...
# Example org (Cloud Platform Tribe). (name, type, products, hardware)
SQUADS = [
    ...
    ("Azure", "product", ["Azure"], []),
    ("GCP / Sovereign", "product", ["GCP", "Sovereign cloud"], []),
    ("AWS", "product", ["AWS"], []),
    ("VMware", "product", ["VMware VCF"], ["Hyperconverged nodes"]),
    ("Edge Computing", "product", ["Cloud Broker", "Edge Gateway", "Data Platform"], []),
...
def main():
    ...
    try:
        ...
        tribe = Tribe(name="Cloud Platform Tribe", description="Cloud & Edge Computing", display_order=1)
        db.add(tribe)
        db.flush()
