...
def test_render_html_follows_lang(db, seeded):
    ...
    assert "Rapport hebdomadaire" in fr and "Squad leader" in fr
    assert "Weekly report" in en and "Squad leader" in en
...
def test_render_pptx_produces_valid_deck(db, seeded):
    ...
    # Summary one-pager + one full detail slide per squad + the closing
    # "attention points" slide of the weekly report.
    assert len(prs.slides) == 1 + n_squads + 1
...
def test_dashboard_pptx_never_silently_drops_squads(db, seeded):
    ...
    # The summary table continues over as many slides as it needs (14 squads on
    # the first, 20 on each next one), then exactly one detail slide per squad.
    summary = 1 + max(0, -(-(n - 14) // 20))
    assert len(prs.slides) == summary + n + 1   # + the attention points slide
...
def test_dashboard_pptx_marks_omitted_squads_when_cap_hit(db, seeded, monkeypatch):
    ...
    n = sum(len(blk["squads"]) for blk in data["tribes"])
    prs = pptx.Presentation(io.BytesIO(render_pptx(data)))
    # 1 summary + 4 detail + the attention points + 1 notice slide.
    assert len(prs.slides) == 1 + 4 + 1 + 1
...
def test_the_dependencies_export_shows_dependencies_inside_a_tribe(client, seeded):
    ...
    Il valait « hors tribe uniquement », et une installation d'une seule tribu n'a
    ...
    assert "hors tribe uniquement" in f.text
...
def _mailbox(db, monkeypatch):
    ...
    monkeypatch.setattr(report_mod, "render_pptx", lambda data: b"")
    monkeypatch.setattr("app.mail.send_email",
                        lambda cfg, to, subject, body, **k: sent.append((to, subject, body)) or True)
