...
def test_an_ordinary_admin_can_reset_the_users_including_themself(client, db, seeded):
    """Only the break-glass account survives a users reset. An admin who is not it
    used to get a 500 after the data was already gone: the audit row pointed at
    the account the reset had just removed, and a commit in the middle had made
    the operation half done."""
    from app.models import AuditLog, User
    from app.security import hash_password
    db.add(User(email="ops@test", display_name="Ops", role="admin", password_hash=hash_password("pw")))
    db.commit()
    login(client, "ops@test")
    r = client.post("/api/admin/data/reset",
                    json={"domains": ["users"], "confirm": True, "snapshot_first": False})
    assert r.status_code == 200, r.text
    row = db.query(AuditLog).filter_by(action="data.reset").one()
    assert row.user_id is None and row.detail["by"] == "ops@test"
