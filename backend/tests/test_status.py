...
def test_counts():
    ...
    c = counts(s, YEAR)
    assert "objectives_red" not in c  # the retired objectives are no longer counted
...
def test_annual_progress_counts_only_the_planned_quarters():
    """All the work sat in Q1-Q2 and is done: that is 100%, not 50%. A quarter with
    nothing planned is neither late nor 0% done."""
    from datetime import datetime, timezone
    from app.status import annual_progress_pct, objective_status
    s = make_squad(roadmap=[(1, "done"), (2, "done")])
    assert annual_progress_pct(s, YEAR) == 100
    obj = SimpleNamespace(year=YEAR, target_date=None)
    assert objective_status(obj, s, datetime(YEAR, 10, 15, tzinfo=timezone.utc)) == "green"

    half = make_squad(roadmap=[(1, "done"), (3, "on_track")])
    assert annual_progress_pct(half, YEAR) == 50          # Q3 planned and not done still counts
    assert annual_progress_pct(make_squad(), YEAR) == 0   # nothing planned at all
