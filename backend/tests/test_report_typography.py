...
def test_the_single_squad_export_too(db, rich):
    ...
    # percentage after its amount, written the French way (narrow no-break space)
    assert "(80 %)" in deck and "(125 %)" in deck
...
def test_every_other_document_too(db, rich):
    ...
    assert "Alice Martin" in inits_html and "30/06/2026" in inits_html
...
def test_the_separators_were_replaced_and_not_simply_dropped(db, rich):
    ...
    # La dependance a sa propre ligne sous le titre du jalon, donc elle n'a plus
    # besoin de parentheses: ce qui compte est qu'elle soit toujours ecrite.
    assert "Dép. Tribe Two" in html
