...
def test_the_timeline_hangs_those_jalons_under_that_commitment(client, seeded, db):
    """La frise range un jalon au mois de l'engagement qu'il tient, pas sous son
    initiative: ce que le comite demande est « qu'est-ce qui tombe ce mois-ci ».
    L'initiative garde son lien et son propre document, elle ne structure plus la
    frise."""
    ...
    posed = {(it["title"], g["month"])
             for g in report_mod.timeline_groups(det) for it in g["items"]}
    # Le jalon qui tient l'engagement est dans la boite de cet engagement, datee
    # de juin; celui qui ne tient rien a la sienne, datee du milieu de son
    # trimestre, la seule date que la donnee porte.
    assert ("Cache des dépendances", 5) in posed
    assert ("Nettoyage des images", 4) in posed
