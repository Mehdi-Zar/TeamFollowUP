...
def test_impersonation_uses_universe_override_for_sovereign_cloud(monkeypatch):
    captured = {}
    monkeypatch.setattr(logexport.google.auth, "default",
                        lambda scopes=None: (_FakeCreds(universe="sovereign-cloud.example"), "proj"))

    def fake_impersonated(source_credentials, target_principal, target_scopes,
                          lifetime=3600, iam_endpoint_override=None):
        captured["override"] = iam_endpoint_override
        return _FakeCreds(universe="sovereign-cloud.example")

    monkeypatch.setattr(logexport.impersonated_credentials, "Credentials", fake_impersonated)
    cfg = {"auth_method": "impersonation", "universe_domain": "sovereign-cloud.example",
    ...
    assert captured["override"] == (
        "https://iamcredentials.sovereign-cloud.example"
...
def test_token_and_universe_explicit_wins(monkeypatch):
    ...
    token, universe = logexport._token_and_universe({"universe_domain": "sovereign-cloud.example"}, logexport.SCOPE_GCS)
    assert token == "tok-refreshed"
    assert universe == "sovereign-cloud.example"       # explicit config beats the creds' own


def test_token_and_universe_from_creds(monkeypatch):
    monkeypatch.setattr(logexport, "_credentials", lambda cfg, scopes: _FakeCreds(universe="sovereign-cloud.example"))
    _, universe = logexport._token_and_universe({}, logexport.SCOPE_BQ)
    assert universe == "sovereign-cloud.example"
