...
def test_squad_objectives_are_retired(client, seeded):
    """One notion of commitment, the OTD: squad objectives are gone, for everyone,
    and turning their switch back on does not bring them back."""
    for who in (seeded["sl_a"], seeded["tribe"], seeded["admin"]):
        login(client, who)
        r = client.post("/api/objectives", json={"squad_id": seeded["squad_a"], "year": YEAR, "title": "Obj"})
        assert r.status_code in (404, 405)  # no such route any more
    client.put("/api/admin/modules-config", json={"squad_content": {"objectives": True}})
    assert "objectives" not in client.get("/api/config").json()["modules"]["squad_content"]


def test_tribe_leader_manages_squads(client, seeded):
