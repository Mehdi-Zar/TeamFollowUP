...
def test_requires_detail_is_saved_on_types(seeded, client):
    login(client, seeded["admin"])
    r = client.post("/api/leaves/types", json={"label": "Autre", "requires_detail": True})
    assert r.status_code == 201 and r.json()["requires_detail"] is True
    tid = r.json()["id"]
    r = client.put(f"/api/leaves/types/{tid}", json={"label": "Autre", "requires_detail": False})
    assert r.status_code == 200 and r.json()["requires_detail"] is False


def test_a_decided_leave_is_not_decided_again(seeded, db, client):
    cp, _ = _seed_types(db)
    _link(db, seeded["member"], seeded["squad_a"])
    login(client, seeded["member"])
    lid = client.post("/api/leaves", json={"type_id": cp, "start_date": "2026-07-06",
                                           "end_date": "2026-07-10"}).json()["id"]
    login(client, seeded["sl_a"])
    assert client.post(f"/api/leaves/{lid}/decision", json={"action": "reject"}).status_code == 200
    assert client.post(f"/api/leaves/{lid}/decision", json={"action": "approve"}).status_code == 409
