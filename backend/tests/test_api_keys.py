...
def test_a_tribe_key_does_not_see_other_tribes_squads_without_a_leader(client, seeded):
    """A key acts with no id: "led by nobody" must not read as "led by me"."""
    _, scoped = _mint(client, seeded, name="t1-only", scopes=["dashboard:read"], tribe_id=seeded["t1"])
    html = client.get("/api/reports/dashboard.html?since_days=7", headers=_auth(scoped)).text
    assert "Squad C" not in html
