...
def test_admin_sees_and_approves_request(client, seeded, db):
    ...
    assert set(reqs["roles"]) == {"admin", "tribe_leader", "squad_leader", "contributor", "member"}
...
def test_tribe_leader_scope(client, seeded, db):
    ...
    assert set(opts["roles"]) == {"squad_leader", "contributor", "member"}
...
def test_a_tribe_leader_places_into_one_of_their_squads(client, seeded, db):
    ...
    login(client, seeded["tribe"])
    opts = client.get("/api/access-requests").json()
    assert opts["roles"] == ["squad_leader", "contributor", "member"] and opts["can_deny"] is True
