...
def test_create_squad_with_products_and_hardware(seeded, client):
    ...
    assert d["products"] == ["Cloud Broker", "Edge Gateway", "Data Platform"]
    assert d["hardware"] == ["Hyperconverged nodes"]


def test_edit_products_and_hardware(seeded, client):
    login(client, seeded["tribe"])
    sa = seeded["squad_a"]
    client.put(f"/api/squads/{sa}", json={"products": ["GCP", "Sovereign cloud"], "hardware": []})
    d = client.get(f"/api/squads/{sa}").json()
    assert d["products"] == ["GCP", "Sovereign cloud"] and d["hardware"] == []
