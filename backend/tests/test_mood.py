...
def test_the_deck_draws_the_cloud_as_shapes(client, db, seeded):
    ...
    # dit seulement "forme automatique" et ne distingue donc pas un ovale d'un carre.
    # Le nuage, lui, n'est pas une forme du logiciel: il est dessine point par point
    # a partir de la meme description que le SVG de l'application, parce que le
    # nuage de PowerPoint a d'autres bosses et faisait donc une autre icone.
    ...
    drawn = [sh for sh in prs.slides[0].shapes
             if sh.shape_type == MSO_SHAPE_TYPE.FREEFORM
             and sh.top < Inches(1.2) and sh.width > Inches(0.3)]
    assert len(drawn) == 1, "le nuage lui-meme"
    assert sum(1 for k in kinds if k.startswith("OVAL")) >= 2, "les deux yeux"


def test_the_slide_and_the_page_draw_the_same_cloud():
    """Une seule description du nuage, deux rendus. Tant que la slide prenait la
    forme « nuage » de PowerPoint, le document montrait une autre icone que
    l'ecran: meme couleur, autres bosses, donc autre logo."""
    from app.reportcommon import (MOOD_CLOUD_BOX, MOOD_CLOUD_D, mood_cloud_outline,
                                  mood_cloud_svg)

    assert MOOD_CLOUD_D in mood_cloud_svg("good"), "le SVG lit la description partagee"
    points = mood_cloud_outline()
    width, height = MOOD_CLOUD_BOX
    assert len(points) > 24, "le contour est echantillonne, pas reduit a ses sommets"
    assert all(-1 <= x <= width + 1 and -1 <= y <= height + 1 for x, y in points), \
        "le contour tient dans le repere de la description"
    # Le contour se referme par le bas plat du nuage: premier et dernier point
    # sont a la meme hauteur, aux deux bouts de cette base.
    assert points[0][1] == points[-1][1]
    assert points[-1][0] - points[0][0] > 20
